...
import type { Report, Status, View } from "./types";
...
import Breadcrumbs from "./components/layout/PagePath";
...
export default function CitizenPortalMVP() {
    ...
    <div className="min-h-screen bg-[#f6f7f9] text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-4 px-4 py-6 md:grid-cols-[240px_1fr]">
        ...
        <main className="min-w-0">
          <Breadcrumbs current={viewTitle(view)} />
          ...
          <div className="mt-3 rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
            <div className="p-4 sm:p-6">
              ...
              {view === "profile" && (
                <Profile
                  onSaved={() => push({ title: "Profile saved (mock)" })}
                />
              )}

              {view === "settings" && (
                <SettingsPage
                  onSaved={() => push({ title: "Settings saved (mock)" })}
                />
              )}
